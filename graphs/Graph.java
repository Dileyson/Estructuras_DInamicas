...
public class Graph {
    ...
    public boolean validateInOut() {
        int[] filasRecorridas = new int[this.totalNodes];
        int[] columnasRecorridas = new int[this.totalNodes];
        boolean resultado;
        ...
        for (int i = 0; i < this.totalNodes; i++) {
            ...
            filasRecorridas[i] = contadorFilas;
            columnasRecorridas[i] = contadorColumnas;
        }

        for (int i = 0; i < this.totalNodes; i++) {
            if (filasRecorridas[i] == columnasRecorridas[i]) {
                validar += 1;
            }
        }

        if (validar == this.totalNodes) {
            resultado = true;
        } else {
            resultado = false;
        }
